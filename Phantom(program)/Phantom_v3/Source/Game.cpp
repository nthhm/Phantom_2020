...
// カメラ関係の定義
#define CAMERA_ANGLE_SPEED				0.10f		// 旋回速度0.05
#define CAMERA_PLAYER_TARGET_HEIGHT		1130.0f		// プレイヤー座標からどれだけ高い位置を注視点とするか400
#define CAMERA_PLAYER_LENGTH			50.0f		// プレイヤーとの距離1600
#define CAMERA_COLLISION_SIZE			5.0f		// カメラの当たり判定サイズ50

// プレイヤーキャラ以外キャラの情報
#define NOTPLAYER_NUM					10			// プレイヤー以外のキャラの数
#define NOTPLAYER_MOVETIME				175			// 一つの方向に移動する時間
...
{
	// モデルの読み込み
	chcmn.BaseModelHandle = MV1LoadModel("dat\\Police.x");
...
void Player_Initialize(void)
{
	...
	//加速度センサ値初期化
	//r = ERS_Open(com, BUFSIZE, BUFSIZE);
...
void Player_Process(void)
{
	...
	/* -->コメント外す　if (ERS_CheckRecv(com) > 0)
	{  // 受信データあり
		r = ERS_Recv(com, buf, 1);
		r = ERS_Send(com, buf, 1);	// 受信データを返す　*/

									// パッドの３ボタンと左シフトがどちらも押されていなかったらプレイヤーの移動処理
		if (CheckHitKey(KEY_INPUT_LSHIFT) == 0 && (inp.NowInput & PAD_INPUT_C) == 0 /*&& buf[0] == 'A'*/ )
		{
...
void NotPlayer_Initialize(void)
{
	...
	static VECTOR FirstPosition[NOTPLAYER_NUM] =
	{
		...
		{ 2800.0f, 0.0f, 2000.0f },
		{ 2000.0f, 0.0f, 2000.0f },
		{ 6000.0f, 0.0f, 2300.0f },
		{ -4500.0f, 0.0f, -2500.0f },
		{ 8600.0f, 0.0f, 5100.0f },
		{ 7800.0f, 0.0f, 6000.0f },
...
{
	// ステージモデルの読み込み
	stg.ModelHandle = MV1LoadModel("dat\\ColTestStage.mqo");
...
void Camera_Process(void)
{
	...
	if (CheckHitKey(KEY_INPUT_LSHIFT) || (inp.NowInput & PAD_INPUT_C))
	{
		...
		if (inp.NowInput & PAD_INPUT_UP)
		{
			...
			// ある一定角度以下にはならないようにする
			if (cam.AngleV < -DX_PI_F / 2.0f + 1.5f)
			{
				cam.AngleV = -DX_PI_F / 2.0f + 1.5f;
			}
		}
		...
		if (inp.NowInput & PAD_INPUT_DOWN)
		{
			...
			// ある一定角度以上にはならないようにする
			if (cam.AngleV > DX_PI_F / 2.0f - 1.2f)
			{
				cam.AngleV = DX_PI_F / 2.0f - 1.2f;
			}
		}
	}
...
void Item_Terminate(void) {
	...
	MV1DeleteModel(item.ItemHandle);
...
void Render_Process(void)
{
	...
	//タイマー描画
	nowTime = limitTime - ((GetNowCount() - startTime) / 1000);
	DrawFormatString(100, 100, GetColor(255, 255, 255), "%d秒", nowTime);
...
void Game_Update() {
	...
	while (ProcessMessage() == 0 && CheckHitKey(KEY_INPUT_ESCAPE) == 0) {
		...
		if (Game_I == 1) {
			SceneMgr_ChangeScene(eScene_Clear);		//シーンをクリア画面に変更
			break;
		}

		if (Game_I == 2) {
			SceneMgr_ChangeScene(eScene_Out);		//シーンをゲームオーバー画面に変更
			break;
		}

		if (nowTime == 0) { //制限時間が0になったら
			SceneMgr_ChangeScene(eScene_Out);		//シーンをゲームオーバー画面に変更
			break;
		}
